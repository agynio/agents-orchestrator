...
replace(
  ...
  [
    ...
    '	}',
    '',
    '	if err := waitForMCPServerPorts(ctx, cfg.MCPServers, mcpReadyTimeout); err != nil {',
    '		tracingProxy.Close()',
    '		_ = setup.gatewayConn.Close()',
    '		return nil, err',
    '	}',
...
replace(
  'internal/daemon/daemon_mcp_test.go',
  'TestWaitForMCPServerPortsReady',
  [
  ...
  [
    ...
    '}',
    '',
    'func TestWaitForMCPServerPortsReady(t *testing.T) {',
    ...
    '	if err := waitForMCPServerPorts(ctx, servers, 500*time.Millisecond); err != nil {',
    '		t.Fatalf("expected MCP server port to be ready, got %v", err)',
    '	}',
    '}',
  ].join('\n'),
);
...
replace(
  ...
  [
    'func waitForMCPServerPorts(ctx context.Context, servers []config.MCPServer, timeout time.Duration) error {',
...
replace(
  'internal/daemon/daemon.go',
  'codex turn completion timeout',
  'const mcpProbeID = "agynd-mcp-ready"',
  ['const mcpProbeID = "agynd-mcp-ready"', '', 'var codexTurnCompletionTimeout = 2 * time.Minute'].join('\n'),
);

replace(
  'internal/daemon/daemon.go',
  'codex turn completion timeout context',
  [
  ...
  [
    '\tcompletionCh := d.tracker.Register(turnID)',
    '\tcompletionCtx, completionCancel := context.WithTimeout(ctx, codexTurnCompletionTimeout)',
...
replace(
  'internal/daemon/daemon.go',
  'codex turn completion timeout retry',
  [
    '\tcase <-ctx.Done():',
  ...
  [
    ...
    '\t\t\t\t\topCodexWaitTurnCompletion,',
    '\t\t\t\t\t0,',
    '\t\t\t\t\tfmt.Errorf("codex turn %s completion timed out after %s for message %s; reset codex thread: %w", turnID, codexTurnCompletionTimeout, message.ID, resetErr),',
    '\t\t\t\t)',
    '\t\t\t}',
    '\t\t\tlog.Printf("codex turn completion timeout: turn_id=%s message_id=%s timeout=%s; reset thread mapping and retrying sync", turnID, message.ID, codexTurnCompletionTimeout)',
    '\t\t\treturn operationError(',
    '\t\t\t\topCodexWaitTurnCompletion,',
    '\t\t\t\t0,',
    '\t\t\t\tfmt.Errorf("codex turn %s completion timed out after %s for message %s", turnID, codexTurnCompletionTimeout, message.ID),',
    '\t\t\t)',
    '\t\t}',
    '\t\treturn operationError(',
    '\t\t\topCodexWaitTurnCompletion,',
...
replace(
  'internal/daemon/daemon_codex_test.go',
  'codex turn completion timeout test',
  'func TestHandleCodexMessageWrapsTurnResultError(t *testing.T) {',
  [
    'func TestHandleCodexMessageTimesOutWaitingForTurnCompletion(t *testing.T) {',
    ...
    '\t}',
    '\toldTimeout := codexTurnCompletionTimeout',
    '\tcodexTurnCompletionTimeout = 20 * time.Millisecond',
    '\tt.Cleanup(func() { codexTurnCompletionTimeout = oldTimeout })',
    '',
    '\terr := daemon.handleCodexMessage(context.Background(), platform.Message{ID: "msg-1", ThreadID: "thread-1", Body: "hello"})',
    '\tif err == nil {',
    '\t\tt.Fatal("expected turn completion timeout")',
    '\t}',
    '\tfor _, expected := range []string{"codex_wait_turn_completion", "completion timed out", "msg-1"} {',
    '\t\tif !strings.Contains(err.Error(), expected) {',
    '\t\t\tt.Fatalf("expected %q in error: %v", expected, err)',
    '\t\t}',
    '\t}',
    '\tif isTerminalAgentProcessingError(err) {',
    '\t\tt.Fatalf("expected timeout to remain retryable, got terminal error: %v", err)',
    '\t}',
    '\tif client.startThreadCalls != 2 {',
    '\t\tt.Fatalf("expected replacement codex thread after timeout, got %d starts", client.startThreadCalls)',
    '\t}',
    '}',
    '',
    'func TestHandleCodexMessageWrapsTurnResultError(t *testing.T) {',
