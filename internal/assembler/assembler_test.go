...
func TestZitiSidecarBypassesImageEntrypointEnrollment(t *testing.T) {
	...
	if !strings.Contains(zitiSidecarScript, `--dnsUpstream "udp://${workload_dns_upstream}:53" --dnsUpstream "tcp://${workload_dns_upstream}:53"`) {
		t.Fatalf("expected sidecar script to forward non-Ziti DNS to workload DNS using supported 2.0.0 flags, got %q", zitiSidecarScript)
	}
